...
final class EncounterNotification {
	...
	private int getPokemonResourceId(int pokemonNumber, MODIFIER modifier) {
		return mResources.getIdentifier("pokemon_" + Strings.padStart(String.valueOf(pokemonNumber), 3, '0') + (modifier != MODIFIER.NO ? "_" + modifier.name().toLowerCase() : ""), "drawable", mContext.getPackageName());
	}
